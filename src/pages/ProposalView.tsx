...
import { ClientDataDialog } from "@/components/ClientDataDialog";
...
const ProposalView = () => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const fileName = `Proposta_${proposal?.client?.company || proposal?.client?.name || "Cliente"}_${format(new Date(), "dd-MM-yyyy")}.pdf`;
      pdf.save(fileName);
  ...
  return (
    <>
      <ClientDataDialog
        ...
        proposalId={id!}
        onClientSaved={() => {
          fetchProposal().then(() => generatePDF());
        }}
      />
